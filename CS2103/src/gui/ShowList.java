...
public class ShowList extends AppPage {
		...
		.addListener(
				(ObservableValue<? extends State> ov,
					...
					if (newState == Worker.State.SUCCEEDED) {						
						...
						// construct JSON to pass to JS and call add task
						addTasks();				
	...
	public class ListBridge {
		...
		public void receiveCommand(String command){
			...
			if(cmd.toLowerCase().equals("help")||cmd.toLowerCase().equals("show help")){
				GUIController.displayHelp();
			}else if(cmd.toLowerCase().equals("change filepath")){				
				changeStorageLocation();
			}else if(cmd.toLowerCase().equals("exit")||cmd.toLowerCase().equals("quit")){
				App.terminate();
			}else{
